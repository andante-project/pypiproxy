...
def ensure_proxy_gets_package_content_from_pypi_if_it_is_not_cached (temp_dir):
    ...
    when(proxy_package_index)._fetch_url(any_value(), raw=any_value()).thenReturn(package_content)
    ...
    verify(proxy_package_index)._fetch_url("http://pypi.python.org/packages/source/p/pyassert/pyassert-0.2.5.tar.gz", raw=True)
    verify(proxy_package_index._package_index).add_package("pyassert", "0.2.5", package_content)
    verify(proxy_package_index._package_index).get_package_content("pyassert", "0.2.5")
